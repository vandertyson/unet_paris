...
FN_MASK_CSV = "/data/train/AOI_2_Vegas_Train/summaryData/AOI_2_Vegas_Train_Building_Solutions.csv"
#FN_MASK_CSV = "./AOI_3_Paris_Train_Building_Solutions.csv"
#FN_RGB = "./out_path/RGB/"
FN_RGB = "/data/train/AOI_2_Vegas_Train/RGB-PanSharpen/"
INPUT_SIZE = 256
...
def prepare_mask_tif():
    ...
    ids = [f[:-4] for f in listdir(FN_RGB) if isfile(join(FN_RGB, f))]
    for image_id in ids[:]:        
        mask = image_mask_resized_from_summary(df_summary, image_id[15:])
       # plt.imshow(mask)
       # plt.show()
        im_path = FN_RGB + image_id + ".tif"
